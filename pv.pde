...
class pt { float x=0,y=0,z=0; 
   ...
   pt moveWithMouse() {y += mouseY-pmouseY;  return this;}; //32
   }
   
// =====  vector functions
vec V() {return new vec(); };                                                                          // make vector (x,y,z)
vec V(float x, float y) {return new vec(x,y,0.0); };                                            // make vector (x,y,z)
...
void v(pt P) {vertex(P.x,P.y,P.z);};                                          // changes normal for smooth shading
void v(pt P, int u, int v) {vertex(P.x,P.y,P.z,u,v);};                                            // vertex for shading or drawing
